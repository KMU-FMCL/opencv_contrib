...
public:
  ...
  std::vector<int> generate_superpixels(cv::Mat image, int spixel_size_,
                                        float spatial_weight_);
};
...
public:
  ...
  Vector2i getImageSize() { return img_size; };
...
protected:
  void cvtImgSpace(cv::Ptr<UChar4Image> inimg, cv::Ptr<Float4Image> outimg);
...
__host__ __device__ __forceinline__ void rgb2CIELab(const Vector4u &pix_in,
  ...
  if (_b <= 0.04045f)
    ...
    _b = pow((_b + 0.055f) / 1.055f, 2.4f);
  if (_g <= 0.04045f)
    _g = _g / 12.92f;
  else
    _g = pow((_g + 0.055f) / 1.055f, 2.4f);
  if (_r <= 0.04045f)
    _r = _r / 12.92f;
  else
    _r = pow((_r + 0.055f) / 1.055f, 2.4f);
...
initClusterCentersShared(const Vector4f *inimg, Vector2i map_size,
                         Vector2i img_size, int spixel_size, int x, int y,
                         cv::hfs::slic::gSpixelInfo *out_spixel) {
...
computeSlicDistance(const Vector4f &pix, int x, int y,
                    const cv::hfs::slic::gSpixelInfo &center_info, float weight,
                    float normalizer_xy, float normalizer_color) {
  float dcolor =
...
__host__ __device__ __forceinline__ void findCenterAssociationShared(
    const Vector4f *inimg, const cv::hfs::slic::gSpixelInfo *in_spixel_map,
    Vector2i map_size, Vector2i img_size, int spixel_size, float weight, int x,
    int y, float max_xy_dist, float max_color_dist, int *out_idx_img) {
  ...
  for (int i = -1; i <= 1; i++)
    for (int j = -1; j <= 1; j++) {
      ...
      if (ctr_x_check >= 0 && ctr_y_check >= 0 && ctr_x_check < map_size.x &&
        ...
        float cdist =
            computeSlicDistance(inimg[idx_img], x, y, in_spixel_map[ctr_idx],
...
__host__ __device__ __forceinline__ void
finalizeReductionResultShared(const cv::hfs::slic::gSpixelInfo *accum_map,
                              Vector2i map_size, int num_blocks_per_spixel,
                              int x, int y,
  ...
  for (int i = 0; i < num_blocks_per_spixel; i++) {
    ...
    spixel_list[spixel_idx].num_pixels += accum_map[accum_list_idx].num_pixels;
  }

  if (spixel_list[spixel_idx].num_pixels != 0) {
    spixel_list[spixel_idx].center /= (float)spixel_list[spixel_idx].num_pixels;
    spixel_list[spixel_idx].color_info /=
        (float)spixel_list[spixel_idx].num_pixels;
  } else {
    spixel_list[spixel_idx].center = Vector2f(-100, -100);
    spixel_list[spixel_idx].color_info = Vector4f(-100, -100, -100, -100);
  }
}

__host__ __device__ __forceinline__ void
supressLocalLable(const int *in_idx_img, Vector2i img_size, int x, int y,
                  int *out_idx_img) {
...
__host__ __device__ __forceinline__ void
supressLocalLable2(const int *in_idx_img, Vector2i img_size, int x, int y,
                   int *out_idx_img) {
...
__host__ __device__ __forceinline__ dim3 getGridSize(Vector2i dataSz,
                                                     dim3 blockSz) {
...
struct Float2_ {
  ...
  __host__ __device__ Float2_(float x_, float y_) { x = x_, y = y_; }
  volatile float x, y;
};
