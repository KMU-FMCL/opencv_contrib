...
public:
  ...
  __CV_CUDA_HOST_DEVICE__ friend Vector2<T> &operator+=(Vector2<T> &lhs,
                                                        const Vector2<T> &rhs) {
    lhs.x += rhs.x;
    lhs.y += rhs.y;
    return lhs;
...
public:
  ...
  __CV_CUDA_HOST_DEVICE__ Vector4(const T v0, const T v1, const T v2,
                                  const T v3) {
    this->x = v0;
    this->y = v1;
    this->z = v2;
  ...
  __CV_CUDA_HOST_DEVICE__ friend Vector4<T> &operator+=(Vector4<T> &lhs,
                                                        const Vector4<T> &rhs) {
    lhs.x += rhs.x;
    lhs.y += rhs.y;
    lhs.z += rhs.z;
