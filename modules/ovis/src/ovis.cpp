...
void _createTexture(const String &name, Mat image) {
  ...
  case CV_32FC1:
    ...
    break;
  default:
    CV_Error(Error::StsBadArg, "currently supported formats are only CV_8UC1, "
                               "CV_8UC3, CV_8UC4, CV_16UC1, CV_32FC1");
    break;
  }
  ...
  if (!tex) {
    tex = texMgr.createManual(name, RESOURCEGROUP_NAME, TEX_TYPE_2D, image.cols,
                              image.rows, MIP_DEFAULT, format);
  }
...
static void _convertRT(InputArray rot, InputArray tvec, Quaternion &q,
                       Vector3 &t, bool invert = false) {
...
static void _setCameraIntrinsics(Camera *cam, InputArray _K,
                                 const Size &imsize) {
  ...
  float fovy =
      atan2(K(1, 2), K(1, 1)) + atan2(imsize.height - K(1, 2), K(1, 1));
...
struct Application : public OgreBites::ApplicationContext,
  ...
  Application(const Ogre::String &_title, const Size &sz, int _flags)
      : OgreBites::ApplicationContext("ovis"), mainWin(NULL), title(_title),
        w(sz.width), h(sz.height), key_pressed(-1), flags(_flags) {
  ...
  bool oneTimeConfig() CV_OVERRIDE {
    Ogre::String rsname = utils::getConfigurationParameterString(
        "OPENCV_OVIS_RENDERSYSTEM", RENDERSYSTEM_NAME);
  ...
  createWindow(const Ogre::String &name, uint32_t _w, uint32_t _h,
  ...
  void locateResources() CV_OVERRIDE {
    ...
    for (auto loc : _extraResourceLocations) {
      const char *type =
          StringUtil::endsWith(loc, ".zip") ? "Zip" : "FileSystem";

      if (!FileSystemLayer::fileExists(loc)) {
        loc = FileSystemLayer::resolveBundlePath(getDefaultMediaDir() + "/" +
                                                 loc);
      }
...
public:
  WindowSceneImpl(Ptr<Application> app, const String &_title, const Size &sz,
                  int _flags)
    ...
    if (flags & SCENE_SEPARATE) {
      ...
      shadergen.addSceneManager(
          sceneMgr); // must be done before we do anything with the scene
      ...
      if (flags & SCENE_SHADOWS) {
        ...
        rs->addTemplateSubRenderState(
            shadergen.createSubRenderState("SGX_IntegratedPSSM3"));
      }
    ...
    if (!app->mainWin) {
      CV_Assert((flags & SCENE_OFFSCREEN) == 0 &&
      ...
      TexturePtr tex = TextureManager::getSingleton().createManual(
          title, RESOURCEGROUP_NAME, TEX_TYPE_2D, sz.width, sz.height, 0,
          PF_BYTE_RGB, TU_RENDERTARGET, NULL, false, flags & SCENE_AA ? 4 : 0);
      rWin = tex->getBuffer()->getRenderTarget();
      rWin->setAutoUpdated(false); // only update when requested
    } else {
      OgreBites::NativeWindowPair nwin =
          app->createWindow(title, sz.width, sz.height);
      rWin = nwin.render;
      if (camman)
  ...
  void setBackground(InputArray image) CV_OVERRIDE {
    ...
    if (tus->getTextureName() != name) {
      RTShader::ShaderGenerator::getSingleton().invalidateMaterial(
          ...
          bgplane->getMaterial()->getName(), RESOURCEGROUP_NAME);
    ...
    for (int j = 0; j < 2; j++) {
      ...
      for (size_t i = 0; i < names.size(); i++) {
        if (!cm.addCompositor(vp, names[i])) {
          LogManager::getSingleton().logError("Failed to add compositor: " +
                                              names[i]);
          continue;
        }
  ...
  void getCompositorTexture(const String &compname, const String &texname,
                            OutputArray out, int mrtIndex) CV_OVERRIDE {
    CompositorManager &cm = CompositorManager::getSingleton();
    ...
    if (!inst)
      CV_Error_(Error::StsBadArg,
                ("no active compositor named: %s", compname.c_str()));
    ...
    cvtColor(mat, mat,
             CV_MAT_CN(dst_type) == 3 ? COLOR_RGB2BGR : COLOR_RGBA2BGRA);
  }
  ...
  void createEntity(const String &name, const String &meshname, InputArray tvec,
                    InputArray rot) CV_OVERRIDE {
  ...
  Rect2d createCameraEntity(const String &name, InputArray K,
                            const Size &imsize, float zFar, InputArray tvec,
                            InputArray rot, const Scalar &color) CV_OVERRIDE {
    ...
    MaterialPtr mat =
        MaterialManager::getSingleton().create(name, RESOURCEGROUP_NAME);
    ...
    node->setOrientation(
        toOGRE); // camera mesh is oriented by OGRE conventions by default
    ...
    return Rect2d(toOGRE_SS[0] * (ext.right - ext.width() / 2) * scale,
                  toOGRE_SS[1] * (ext.bottom - ext.height() / 2) * scale,
                  ext.width() * scale, ext.height() * scale);
  }

  void createLightEntity(const String &name, InputArray tvec, InputArray rot,
                         const Scalar &diffuseColour,
  ...
  void updateEntityPose(const String &name, InputArray tvec,
                        InputArray rot) CV_OVERRIDE {
    SceneNode &node = _getSceneNode(sceneMgr, name);
    Quaternion q;
    Vector3 t;
  ...
  void setEntityPose(const String &name, InputArray tvec, InputArray rot,
                     bool invert) CV_OVERRIDE {
    SceneNode &node = _getSceneNode(sceneMgr, name);
    Quaternion q;
    Vector3 t;
  ...
  void getEntityPose(const String &name, OutputArray R, OutputArray tvec,
                     bool invert) CV_OVERRIDE {
  ...
  void getEntityAnimations(const String &name,
                           std::vector<String> &out) CV_OVERRIDE {
    SceneNode &node = _getSceneNode(sceneMgr, name);
    Entity *ent = dynamic_cast<Entity *>(node.getAttachedObject(name));
    CV_Assert(ent && "invalid entity");
    for (auto const &anim :
         ent->getAllAnimationStates()->getAnimationStates()) {
      out.push_back(anim.first);
    }
  }

  void playEntityAnimation(const String &name, const String &animname,
                           bool loop = true) CV_OVERRIDE {
    SceneNode &node = _getSceneNode(sceneMgr, name);
    Entity *ent = dynamic_cast<Entity *>(node.getAttachedObject(name));
    CV_Assert(ent && "invalid entity");
    ...
    if (frameCtrlrs.find(animstate) != frameCtrlrs.end())
      return;
    frameCtrlrs.insert(
        {animstate, Ogre::ControllerManager::getSingleton()
                        .createFrameTimePassthroughController(
                            Ogre::AnimationStateControllerValue::create(
                                animstate, true))});
  }

  void stopEntityAnimation(const String &name,
                           const String &animname) CV_OVERRIDE {
    SceneNode &node = _getSceneNode(sceneMgr, name);
    Entity *ent = dynamic_cast<Entity *>(node.getAttachedObject(name));
    CV_Assert(ent && "invalid entity");
    AnimationState *animstate = ent->getAnimationState(animname);
    ...
    animstate->setTimePosition(0);
    Ogre::ControllerManager::getSingleton().destroyController(
        frameCtrlrs[animstate]);
    frameCtrlrs.erase(animstate);
  }

  void setEntityProperty(const String &name, int prop, const String &value,
                         int subEntityIdx) CV_OVERRIDE {
    CV_Assert(prop == ENTITY_MATERIAL);
    SceneNode &node = _getSceneNode(sceneMgr, name);

    MaterialPtr mat =
        MaterialManager::getSingleton().getByName(value, RESOURCEGROUP_NAME);
  ...
  void setEntityProperty(const String &name, int prop,
                         const Scalar &value) CV_OVERRIDE {
    SceneNode &node = _getSceneNode(sceneMgr, name);
    switch (prop) {
    ...
    case ENTITY_ANIMBLEND_MODE: {
      ...
      ent->getSkeleton()->setBlendMode(
          static_cast<Ogre::SkeletonAnimationBlendMode>(int(value[0])));
      break;
    }
    default:
  ...
  void getEntityProperty(const String &name, int prop,
                         OutputArray value) CV_OVERRIDE {
    SceneNode &node = _getSceneNode(sceneMgr, name);
    switch (prop) {
    case ENTITY_SCALE: {
  ...
  void _createBackground() {
    ...
    Mat_<Vec3b> img =
        (Mat_<Vec3b>(2, 1) << Vec3b(2, 1, 1), Vec3b(240, 120, 120));
    ...
    bgplane->setUVs(pc, Vector2(pc[0], 1 - pc[1]), Vector2(1 - pc[0], pc[1]),
                    Vector2(1, 1) - pc);
  ...
  void getDepth(OutputArray depth) CV_OVERRIDE {
    ...
    if (!depthRTT) {
      // render into an offscreen texture
      // currently this draws everything twice as OGRE lacks depth texture
      // attachments
      TexturePtr tex = TextureManager::getSingleton().createManual(
    ...
    PixelBox pb(depthRTT->getWidth(), depthRTT->getHeight(), 1, PF_DEPTH,
                tmp.ptr());
    depthRTT->update();
  ...
  void setCameraIntrinsics(InputArray K, const Size &imsize, float zNear,
                           float zFar) CV_OVERRIDE {
    Camera *cam = sceneMgr->getCamera(title);
  ...
  void setEntityLookAt(const String &origin, const String &target,
                       InputArray offset) CV_OVERRIDE {
    ...
    if (target.compare("") != 0) {
      SceneNode *tgt = sceneMgr->getEntity(target)->getParentSceneNode();
      orig->lookAt(tgt->_getDerivedPosition() + _offset, Ogre::Node::TS_WORLD,
                   Ogre::Vector3::UNIT_Z);
    } else {
...
CV_EXPORTS_W void addResourceLocation(const String &path) {
  ...
  _extraResourceLocations.insert(
      Ogre::StringUtil::normalizeFilePath(path, false));
}

Ptr<WindowScene> createWindow(const String &title, const Size &size,
                              int flags) {
  if (!_app) {
...
void setMaterialProperty(const String &name, int prop, const Scalar &val) {
  ...
  MaterialPtr mat =
      MaterialManager::getSingleton().getByName(name, RESOURCEGROUP_NAME);
...
static TextureUnitState *_getTextureUnitForUpdate(const String &material,
                                                  int prop) {
  CV_Assert_N(prop >= MATERIAL_TEXTURE0, prop <= MATERIAL_TEXTURE3, _app);

  CV_Assert(_app);
  auto mat =
      MaterialManager::getSingleton().getByName(material, RESOURCEGROUP_NAME);
  CV_Assert(mat);
  ...
  RTShader::ShaderGenerator::getSingleton().invalidateMaterial(
      RTShader::ShaderGenerator::DEFAULT_SCHEME_NAME, material,
      RESOURCEGROUP_NAME);
...
static bool setShaderProperty(const GpuProgramParametersSharedPtr &params,
                              const String &prop, const Scalar &value) {
  const GpuConstantDefinition *def =
      params->_findNamedConstantDefinition(prop, false);
...
void setMaterialProperty(const String &name, const String &prop,
                         const Scalar &value) {
  CV_Assert(_app);
  ...
  if (rpass->hasGpuProgram(GPT_FRAGMENT_PROGRAM)) {
    GpuProgramParametersSharedPtr params =
        rpass->getFragmentProgramParameters();
    set = set || setShaderProperty(params, prop, value);
  }

  if (!set)
    CV_Error_(Error::StsBadArg,
              ("shader parameter named '%s' not found", prop.c_str()));
}

void updateTexture(const String &name, InputArray image) {
  CV_Assert(_app);
  TexturePtr tex =
      TextureManager::getSingleton().getByName(name, RESOURCEGROUP_NAME);
  CV_Assert(tex);
  _createTexture(name, image.getMat());
}
} // namespace ovis
